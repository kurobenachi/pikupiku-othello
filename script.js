...
// 音声のロード
const placingPiece = new Audio('./sound/SE/placingPiece.mp3');
const pushingButton = new Audio('./sound/SE/pushingButton.mp3');
const BGM = new Audio('./sound/BGM/FC/FC_smashedBeanPaste.mp3');
...
$(function () {
  ...
  $(".initialize").click(initializeEvent);

  // BGMボタンのイベント設定
  $("#img-btn-BGM").click(toggleBGM);
...
function toggleBGM() {
  if (isBgmPlaying) {
    BGM.pause();
    $("#img-btn-BGM").addClass("btn-valid");
    $("#img-btn-BGM").removeClass("btn-invalid");
  } else {
    BGM.play();
    $("#img-btn-BGM").addClass("btn-invalid");
    $("#img-btn-BGM").removeClass("btn-valid");
  }
...
function putPiece(targetSquare, owner) {
  ...
  //画像をロード
  const pieceImagePath = "./images/pikupikun.png";

  //表示・配置済みにする
  targetSquare.html(`<img src="${pieceImagePath}" alt="${owner} piece" class="piece-image ${owner}">`).attr("data-owner", owner).addClass("selected");
...
function updateTurnIndicator() {
  const turnName = isOddTurn ? "黒" : "白";
  $("#turnIndicator").text(`${turnName}の手番だよ`);
}

function updateTurnNumber() {
  $("#turnNumber").text(`ターン数：${turnNumber}`);
}
...
function playPushingButton() {
  pushingButton.currentTime = 0; // 再生位置を先頭にリセット
  pushingButton.play(); // 効果音を再生
}

function playPlacingPiece() {
  placingPiece.currentTime = 0; // 再生位置を先頭にリセット
  placingPiece.play(); // 効果音を再生
}
...
function playBGM() {
  BGM.currentTime = 0; // 再生位置を先頭にリセット
  BGM.play(); // BGMを再生
}
